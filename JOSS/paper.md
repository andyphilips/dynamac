...
Specifically, dynamac implements six new visualizations. These can be presented one at a time using the command `dynardl.simulation.plot()`. Users wishing to present all six at once can do so with the command `dynardl.all.plots()`. The resulting plots are:
