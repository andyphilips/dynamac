...
* For the levels -----
qui if "`level'" != ""	{					// any vars in levels?
	local levellist : subinstr local level "," `"" ""', all
	local levellist `""`levellist'""'
	loc n_levels : word count `levellist'
	if "`n_levels'" != "`n_vars'" {			// does # levels entries == # variables?
		di in r _n "Level list has `n_levels' entries, but `n_vars' variables specified in [varlist]."
		di in r _n "Add entry for each variable, separated by a comma (missing is '.')"
		exit 198
	}
	local i 1
